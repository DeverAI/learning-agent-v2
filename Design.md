...
- 容量诚实：题量、文件数量或模型能力不足时明确降级或停止，不伪造成功结果。
  **落点要求（2026-09-11 补充）**：兜底/降级值必须与正常值**可区分**（本项目用 `degraded: true`），否则会静默污染下游判定与长期记忆；「没有数据」不得冒充「数据正常」，「缺失」不得按 0 参与统计。
...
### 3.2 代码结构（WEB 服务端 + 桌面端）

系统包含 WEB 服务端与桌面端（PyQt）两个组件，按目录划分；早期独立的桌面端工具（`tools/` SVG Picker）已不在本项目中维护：
...
| 桌面端 | `desktop/` | PyQt 文化课桌面工具（2026-09-09 自 `all_projects/学习搭子桌面版` 并入，源目录保留）：课堂监听/AI 补讲（core/classroom_*）、讲题引擎（core/lecture_engine.py）、TTS（core/tts_player.py）、屏幕标注、课堂笔记经 `/api/notes` 同步到服务端；自带文档体系（desktop/Design.md 等），运行时数据（desktop/data/，含密钥）不入库 |
...
识别链路里有若干**无法避免**的降级：整页切题失败只能按单题处理；多图一题时其中一张图识别失败，只能拿剩下的拼题干。这些降级本身可以接受，**但不允许用户看不见**。

- **降级必须留痕在持久位，不能只写短生命周期文件。**
  `storage/task_states/{qid}.json` 会在任务正常完成时被清除，把降级标记写在那里等于"必然消失"。降级标记统一写 `Question.audit_flags`，前端在列表页渲染成具名徽标。
- **当前两类降级标记**（新增类型时必须同步登记，见下条）：
  - `split_fallback`：整页切题未成功、已按单题处理。提示用户"这条可能是整页多题，建议核对后手动拆题"。
  - `ocr_partial_failure`：部分图片识别失败，题干或参考答案可能不完整。此类**仍判 `done`**（内容可用），靠标记而非状态表达不完整。
- **新增标记类型的登记义务**：`audit_service.FLAG_TYPES` 与 `_PRESERVED_FLAG_TYPES` 必须同时加入。
  后者是自动重写流程的**保留白名单**——不加入的标记会在夜间巡检时被清掉，表现为"标记出现过一会儿又没了"，且排查时不会联想到重写流程。
- **留痕失败不得拖垮主流程**：写标记单独 `try/except` 并记日志，不能因为"记不下来"让识别本身失败。
...
#### 6.3.1 SVG 消毒：按"承载了什么信息"定白名单，不要按属性名定

所有落盘 SVG 都经 `_sanitize_svg`（`_write_svg` 是统一落盘口）。消毒用标签与属性双白名单，拒绝一切外部资源。

**`style` 必须保留（按声明过滤）**：坐标推理与折线示意两条路径产出的线段/圆，**笔画只写在 `style` 里**（`style="stroke:#333;stroke-width:2;fill:none"`）。曾因白名单里没有 `style` 而整条删除，落盘变成 `<line ... />` —— 没有 `stroke` 的 `<line>` 等价于 `stroke:none`，**线完全不显示**，而接口、DB、前端全报成功。

- 允许的 CSS 声明：`stroke/stroke-width/stroke-dasharray/stroke-linecap/stroke-linejoin/stroke-opacity/stroke-miterlimit/fill/fill-opacity/fill-rule/opacity/font-size/font-family/font-style/font-weight/text-anchor/dominant-baseline/letter-spacing/marker-start|mid|end/vector-effect`
- 值里出现 `http(s):`、`data:`、`javascript:`、`expression(`、`@import`、`behavior:`、`-moz-binding` 一律丢弃；`url(...)` 只允许 `url(#id)` 且仅限 `marker-*`
- 实现：`diagram_service.sanitize_style_attr()`；两处消毒调用点共用同一实现

**一般规则**：白名单要按"这个属性承载了什么信息"来定，而不是按"属性名危不危险"。删掉一个承载绘制信息的属性，等于把整个功能静默关掉。

#### 6.3.2 三角函数与对数：约定必须写进给模型的说明

**同仓库存在两套三角函数约定，各自都要写明，不要互相看齐**：

| 模块 | 用途 | 约定 |
|---|---|---|
| `coord_engine.eval_expression` | 图纸坐标表达式 | **度数制**（`sin(30)` = 0.5），反三角返回度数 |
| `calc_service`（calculator 工具） | AI 解题过程中的精确计算 | **弧度制**（走 `math.sin`），`log`/`ln` **都是自然对数** |

给模型的 schema 与解题提示词里必须显式写明单位与底数，并提供显式换算函数（`radians`/`degrees`/`log10`/`lg`）——
否则模型会按角度制写 `sin(30)`，拿到 `-0.988`，而这个错值会直接进入给学生的解题步骤。
...
- 每次讲解段落末尾带「对吧」检查点标记。**该标记由系统保证，不是对 AI 的期望**（2026-09-11 校正）：`focus_service` 在段落生成后检查末尾 10 字，若无「对吧」则**由后端补齐**，且每段的 `has_checkpoint` 恒为 `True`——即 AI 是否主动输出该标记不影响结果。
...
- **开关与降级**：受 `ENABLE_FOCUS_BLACKBOARD` 控制，关闭时前端不渲染黑板、AI 输出不含板书指令。
  **降级的真实边界（2026-09-11 校正）**：只有「AI 返回了合法 JSON 但缺 `board` 字段」时才会**板书留空而教学继续**；若 JSON 解析失败或 `content` 为空，整段会被标记为 `failed`（`_generate_teaching_segment` 的 except 分支），学生需点确认触发重试，**教学流程会中断**。原文「整体降级为纯文本讲解、不影响教学流程」与实现不符。
