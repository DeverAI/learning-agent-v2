...
# 2026-09-12 R23 新增模式（九大重点检修）

## [调用点缺失] 实现完整 + 单测通过 + **生产零调用点** = 功能不可达，且测试全绿时完全隐身
- **错误描述**：本轮一次检修里撞到**两条同型**：
  1. `InterruptEngine.request_teaching`（学生主动打断讲课）：实现完整、round55 就有单测
     （T15 覆盖空请求/静音/冷却旁路），但全仓 `grep request_teaching` 只命中的是
     `demo_coach.py` 与测试 —— **生产代码里一个调用点都没有**。
     于是「随时打断网课并讲解」这话在实现上是不可达的：学生按任何键都不会发生。
  2. `DiagramService._validate_svg_quality`（70 行 SVG 质量自检）：零调用点（R22 已核实）。
     后果是「消毒把 style 删掉 → 图上一条线都看不见」这类事故**没有任何自动检查能发现**。
  **为什么特别难发现**：单元测试是**直接调函数**的，永远不需要"生产里有人调它"这一步。
  所以这类缺陷在"全绿"的仪表盘上 100% 隐身，只有把「声明 → 实现 → 调用点」三段都查一遍才会暴露。
- **正确做法**：
  1. 做完一个能力后，**必须 grep 它的生产调用点**，并把结果写进记录（"入口在 X"或"暂无入口"）；
  2. 给"必须有入口"的能力补一条**入口测试**：断言 UI/热键/路由里存在调用点
     （本轮 `test_r23_*::test_agent_html_auto_recalls_exactly_once`、`desktop/test_r23_ask.py` 就是干这个的）；
  3. 反过来，**发现死代码时先问"它本来该被谁调用"** —— 本轮两条死代码，一条是缺功能，一条是缺检查。
- **本轮出现**：2026-09-12 R23。

## [跨文件消息契约] 前端拼一句中文、后端用正则解析 → 迟早分叉（提成单一来源）
- **错误描述**：本轮实现"后台任务完成 → 召回 Agent"时，前端拼
  `'[系统召回] 后台任务已完成，task_id=' + id`，后端用
  `startswith("[系统召回]")` + 正则 `task_id[=:\s]*([A-Za-z0-9_-]{6,64})` 解析。
  这种"约定字符串"一旦两边各写一份，改一边（加空格、换词）就会**静默失效**——
  召回消息退化成普通对话，用户看到的是"Agent 没被召回"，没有任何报错。
- **正确做法**：把前缀与解析正则提成**后端模块常量**（`sessions._RECALL_PREFIX` / `_RECALL_TASK_ID_RE`），
  测试从**同一个来源**取常量，同时断言"前端源码里用的是这个前缀"和"该正则能解析出前端拼出的消息"。
  两侧都对着同一来源断言，分叉当天就会红。
- **本轮出现**：2026-09-12 R23。

## [硬编码默认口令做兜底] `localStorage.getItem(x) || '真实密码'` —— 改密码即全站不可用
- **错误描述**：前端 API 封装把 token 写成常量默认值（`_authToken: "Ntmhzsgtc"`），
  后端 `password_guard` 在设置了 `api_password` 时对所有 `/api/*` 强制比对。
  两者只在"密码恰好还是默认值"时吻合。本机实测正是这种情况（所以一直看起来正常），
  一旦用户改密码：**整个前端 + 安卓端全部 401，而且没有任何登录页可以输入新密码**。
  另外多处 `|| 'Ntmhzsgtc'` 的兜底把真实口令散落在模板与 JS 里。
- **正确做法**：
  1. token 先读 `localStorage`，硬编码值只作最后兜底；
  2. 401 时**弹一次输入框 → 存 localStorage → 重试一次**（`_retried` 防死循环、
     `_reauthPromise` 防并发弹窗）；401 来自中间件、端点尚未执行，故重试不会重复提交业务操作；
  3. 不要把真实口令写进 `||` 兜底（改成空串/变量引用），兜底值本身不该是一个"能过鉴权的秘密"。
- **本轮出现**：2026-09-12 R23。

## [平台硬上限] TextToSpeech 单段 4000 字符，超了**静默失败**
- **错误描述**：安卓桥 `speak(text)` 直接把整段讲解词丢给 `TextToSpeech.speak`。
  `TextToSpeech.getMaxSpeechInputLength()` 是 4000 字符，**超限不报错、就是不说**：
  长讲解词的后半段没了，界面没有任何提示（学生以为"就读到这儿"）。
- **正确做法**：按平台上限切分（取 3000 留余量），**优先在句子边界下刀**（`。！？；` 与换行），
  多段用 `QUEUE_ADD` 顺序入队；常量必须**小于**平台上限（测试里断言这一点）。
- **本轮出现**：2026-09-12 R23。

## [异步初始化竞态] 桥/引擎就绪前到达的调用被**静默丢弃**
- **错误描述**：`TextToSpeech` 的初始化是异步回调。讲课页一进页面就可能 `speak`，
  而桥里写的是 `if (ttsReady && nativeTts != null) { speak(...) }` —— 没就绪就**直接丢掉**，
  用户看到的现象是"第一句不读"，且没有任何提示（和上面那条合起来，长稿是"头尾都缺"）。
- **正确做法**：加一个待发队列，未就绪先入队；初始化成功回调里 `flush` 补发；
  并且"是否正在朗读"的判断要把队列算进去（`isSpeaking() = 播放中 || 队列非空`），
  否则上层会以为已经讲完。
- **本轮出现**：2026-09-12 R23。

## [质量检查变门禁的误杀风险] 判据先写清适用域，再决定"拦"还是"记"
- **错误描述**：给 SVG 质量自检接线时，若直接拿它当门禁，会把**合法的彩色实验图**拦下：
  它的数学图分支要求"只准黑白灰"，而实验图（液体、火焰、试剂）本来就是彩色的。
- **正确做法**：先判**适用域**（`_is_experiment_svg()`：米白底色 / 带 template / 带 components），
  再决定处置方式。默认取**只观测不拒收**（记日志 + 通过 API 暴露），
  理由是"误杀一张合法图"比"放过一张可疑图"更伤用户；要升级成门禁必须先把判据的误杀率测出来。
- **本轮出现**：2026-09-12 R23。

# 2026-09-12 R25 新增模式

## [anyio.run 不能吃 coroutine 对象] 测试里 anyio.run(_run()) 整页 TypeError
- **错误描述**: anyio.run 签名是 run(func, *args)，要求可调用；anyio.run(_run()) 其中 _run() 已返回 coroutine → 'coroutine' object is not callable。
- **正确做法**: 测试统一用 asyncio.run(coro)，或 anyio.run(_run) 不要提前调用。

## [list_tasks 返回 dict 不是 list] 消费方按 list 遍历导致永远没有运行中任务
- **错误描述**: background_agent.list_tasks 返回 {total,returned,tasks:[]}；打断判定若 for t in active 会遍历 dict 键，主键字段是 task_id 不是 id。
- **正确做法**: 消费 BA 接口前先读 _task_dict 签名；字段名以 task_id 为准。

## [插入函数时误删 def 行] edit 替换目标函数名时把定义一起吃掉
- **错误描述**: 在 async def _session_chat_impl 上方插入新函数，old_string 含原 def 行且 new_string 漏写，函数体悬空。
- **正确做法**: 插入后立刻 Read 核对 def 是否还在；改前备份；失败先怀疑文件版本。
